...
export default function EncargadoDashboardPage() {
  ...
  useEffect(() => {
    ...
    const checkAuth = async () => {
      ...
      if (profile.role !== 'encargado' && profile.role !== 'admin') {
        router.push('/login');
        return;
      }
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-left">
                      <thead className="bg-slate-50 text-[10px] font-bold">
                        <tr><th className="p-4">Documento</th><th className="p-4">Fecha</th><th className="p-4">Tamaño</th><th className="p-4 text-right">Acción</th></tr>
                      </thead>
                      <tbody>
