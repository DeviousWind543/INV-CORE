...
import {
  ...
  Plus, X, MapPin, RefreshCcw, Landmark, Receipt, Trash2, Edit3, Search, Menu, Printer, Camera, EyeOff, Upload, Loader2, LogOut, Clock, Download, FileText, CheckCircle, AlertCircle, Info, AlertTriangle, Trash
} from 'lucide-react';
...
// ======================== COMPONENTE PRINCIPAL PARA ENCARGADO ========================
        ...
        <head>
          ...
          <style>
            * { margin: 0; padding: 0; box-sizing: border-box; }
            ...
            .fecha { text-align: right; font-size: 11px; color: #94a3b8; padding: 15px 30px; background: #f8fafc; border-bottom: 1px solid #e2e8f0; }
            .stats { display: flex; justify-content: space-around; padding: 20px 30px; background: #f1f5f9; gap: 15px; flex-wrap: wrap; }
            .stat-card { background: white; border-radius: 16px; padding: 12px 24px; text-align: center; box-shadow: 0 2px 5px rgba(0,0,0,0.05); flex: 1; min-width: 100px; }
            .stat-number { font-size: 28px; font-weight: 800; color: #1e293b; }
            .stat-label { font-size: 11px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px; color: #475569; margin-top: 4px; }
            h2 { font-size: 18px; margin: 25px 30px 15px 30px; padding-left: 12px; border-left: 5px solid #FFD700; color: #0f172a; font-weight: 700; }
            .table-wrapper { overflow-x: auto; margin: 0 25px 30px 25px; border-radius: 16px; border: 1px solid #e2e8f0; background: white; }
            table { width: 100%; border-collapse: collapse; font-size: 12px; }
            th { background: #f1f5f9; padding: 12px 8px; text-align: left; font-weight: 700; text-transform: uppercase; font-size: 11px; letter-spacing: 0.5px; color: #334155; border-bottom: 1px solid #cbd5e1; }
            td { vertical-align: top; }
            .footer { text-align: center; padding: 20px 30px; background: #f8fafc; border-top: 1px solid #e2e8f0; font-size: 10px; color: #64748b; }
          </style>
        </head>
        <body>
          ...
          <div className="p-4 md:p-8">
            <header className="bg-white rounded-2xl p-5 mb-6 flex justify-between items-center shadow-sm border border-slate-100">
              ...
              <div className="hidden sm:block">
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Panel Encargado</p>
                <h2 className="text-[#2D1B69] font-black">{adminProfile?.full_name || 'Encargado'}</h2>
              </div>
              <div className="flex items-center gap-3">
                {/* Solo estado de Drive, sin botón para conectar */}
                <div className={`px-4 py-2 rounded-lg text-xs font-bold flex items-center gap-2 ${
                  driveChecking ? 'bg-gray-100 text-gray-500' :
                  driveConnected ? 'bg-green-100 text-green-700 border border-green-300' :
                  'bg-red-50 text-red-600 border border-red-200'
                }`}>
                  {driveChecking ? <Loader2 size={14} className="animate-spin" /> :
                   driveConnected ? <><CheckCircle size={14} /> Activo</> :
            ...
            {/* IMPRESIONES (se mantiene igual que en admin) */}
            ...
            {/* MODAL DE ARTÍCULO (completo, igual que admin) */}
            {showModal && (
